...
| Section reveal | Replays on **every** entry — `sectionReveal` toggles `.is-revealed` on the panel root off `activeSlide`; double-rAF paints the hidden baseline first, and leaving resets after `600ms` |
| Staggered fade-up | `.reveal-fade` — `opacity 0 → var(--o, 1)`, `translateY(10px) → 0`, `--dur-reveal`, per-element delay via inline `--d` (ladder tops out at `0.34s`) |
| Line reveal | `.reveal-line` overflow mask with an inner span translated `105%` |
| Panel slide-in | ±3% translateX + opacity, `--dur-reveal`, right half delayed `0.08s` |
...
| Reduced motion | Movement removed, feedback kept: animations to `0.01ms` / one iteration, `transition-property` narrowed to colour · background · border · outline · fill · stroke · opacity · shadow at `150ms` on `--ease-out`. `.reveal-fade` keeps the fade and drops the travel and the stagger |
