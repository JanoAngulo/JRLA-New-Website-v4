...
The register is **precise, technical, and tactile**. Precision shows in the labels — IBM Plex Mono at 10px with 0.3em tracking, uppercase, always prefixing a value, never floating on its own. Technical shows in what the surface refuses: no gradients as decoration, no glass, no soft toy-like rounding, no illustration standing in for evidence. Tactile shows in the response — everything pressable depresses to `scale(0.95)` in 80ms, cards lift 3px and sharpen their border to the accent, the theme toggle overshoots on a spring. The interface behaves like machined controls, and the display type is the one place it raises its voice.
...
- Immediate physical feedback on every interactive element
...
- **Display** (ExtraBold, `clamp(3.5rem, 12vw, 11rem)`, line-height 0.85, tracking −0.03em): The hero name only. Sized to be read across a room; tracking pulled tight so the mass reads as one object.
...
- **Fully round** (`9999px`) — pills. Filter pills, social pills (`2.75rem` circles), the drag handle on the mobile sheet, status dots. Anything that reads as a control rather than content.
- **Small radius** (`6px` inputs, `8px` tool chips) — form fields and the square tool-logo chips. Just enough to say "this accepts interaction" without softening the page.
- **`4px`** — focus rings only, so the outline hugs mixed-shape targets consistently.

The old `.card` / `.card-left` / `.card-right` helpers (`rounded-xl`) have been **deleted** — nothing used them, and rounded content plates contradict the rule above. The remaining `rounded-2xl` usage is confined to `SkeletonLoader` placeholder blocks and is not part of the shape language.
...
- **Press:** `scale(0.95)` in `80ms` on `:active`, gated behind `prefers-reduced-motion: no-preference`.
...
- **Theme toggle:** `2.25rem` circle, hairline border that becomes accent on hover, icon swapped on a `500ms` spring (`cubic-bezier(0.34, 1.56, 0.64, 1)`), `scale(1.05)` hover / `scale(0.90)` press. Cycles system → light → dark.
- **Mobile:** Minimum `2.75rem` hit targets; `active:scale-95` on the brand mark.
...
- **Do** collapse motion under `prefers-reduced-motion: reduce` — durations to `0.01ms` globally, reveals to a `0.2s` opacity-only fade.
...
- **Don't** use the accent as a large surface fill. It marks state, not area — the 10%/8% hero radial wash is the only exception.
...
- **Don't** add a third easing family. Reveals and presses use `cubic-bezier(0.22, 1, 0.36, 1)`; scroll and layout use `cubic-bezier(0.2, 0.8, 0.2, 1)`. The overshoot spring `cubic-bezier(0.34, 1.56, 0.64, 1)` is reserved for **momentary single-element confirmations** — the theme-toggle icon swap and the submit checkmark pop — and nothing that carries content.
...
Two easing families split by purpose, plus one sanctioned spring:

| Curve | Uses | Notes |
|---|---|---|
| `cubic-bezier(0.22, 1, 0.36, 1)` | Reveals, presses, hover lifts, tool/social pills | 30 occurrences — the dominant curve |
| `cubic-bezier(0.2, 0.8, 0.2, 1)` | Scroll-linked and layout transitions | 17 occurrences |
| `cubic-bezier(0.34, 1.56, 0.64, 1)` | Momentary confirmations only — theme-toggle icon swap, submit checkmark pop | Overshoot is the point; never on content |
| `cubic-bezier(0.65, 0, 0.35, 1)` | One scoped in-out transition | Not a system curve |

| Pattern | Spec |
|---|---|
| Horizontal scrub | GSAP ScrollTrigger `scrub: 1`; free-form snap with a `0.06` dead-zone; off-centre panels depth-scale to `1 − 0.05·smoothstep(dist)` |
| Pause-and-pan | Tall panels lock horizontal travel and pan `[data-pan-scroll]` to its end before releasing |
| Section reveal | Replays on **every** entry — `sectionReveal` toggles `.is-revealed` on the panel root off `activeSlide`; double-rAF paints the baseline first |
| Staggered fade-up | `.reveal-fade` — `opacity 0 → var(--o, 1)`, `translateY(10px) → 0`, `0.7s`, per-element delay via inline `--d` |
| Line reveal | `.reveal-line` overflow mask with an inner `.hl-inner` per heading line |
| Slide-in | `0.9s`, ±3% translateX, `0.15s` stagger |
| Typed text | 80ms/char · 2200ms hold · 40ms delete · 1s blink |
| Tactile press | `scale(0.95)` in `80ms` on `:active` |
| Card lift | `translateY(-3px)`, image `scale(1.04)`, `300ms` |
| Status pulse | `scale 1 → 1.4`, `opacity 0.9 → 0.4`, `2s` loop |
| Sheet dismiss | Drag Y past threshold → close |
| Reduced motion | All durations `0.01ms !important`; `.reveal-fade` → `0.2s`, no transform, no delay |
...
Dark mode swaps: accent (Signal Red → Filament Yellow), page and card surfaces, tool-logo SVG variants (html/css/js/nextjs/wacom), focus-ring colour, and the `::selection` foreground.
...
- `prefers-reduced-motion` honoured globally in `@layer base`.
- Landmarks: `nav`, main slide region, `role="dialog"` + `aria-modal`.
...
- **Imagery** — `.webp` preferred; first 3 images of any grid eager-loaded, the rest lazy via `LazyImage` (IntersectionObserver, shimmer until decoded, `width`/`height` for CLS).
...
- `<style scoped>` only for one-off visuals (nav arrows, scoped reveal keyframes, marquee).
...
- New animation? Add its curve and duration to §Motion. New category? Extend the badge pairs and the data enum.
